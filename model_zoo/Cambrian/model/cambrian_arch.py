...
from model_zoo.Cambrian.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN

from model_zoo.Cambrian.utils import IS_XLA_AVAILABLE
...
class CambrianMetaForCausalLM(ABC):
    ...
    def encode_images(self, image_aux_list):
        """
        (Pdb) type(vision_tower_aux_list[0])
            <class 'model_zoo.Cambrian.model.multimodal_encoder.siglip_encoder.SiglipVisionTower'>
        (Pdb) type(vision_tower_aux_list[1])
            <class 'model_zoo.Cambrian.model.multimodal_encoder.clip_encoder.ClipVisionTower'>
        (Pdb) type(vision_tower_aux_list[2])
            <class 'model_zoo.Cambrian.model.multimodal_encoder.dino_encoder.DinoVisionTower'>
        (Pdb) type(vision_tower_aux_list[3])
            <class 'model_zoo.Cambrian.model.multimodal_encoder.clip_convnext_encoder.CLIPConvNextTower'>
        """
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if vision_tower_aux_list is None or images is None or input_ids.shape[1] == 1:
            if past_key_values is not None and vision_tower_aux_list is not None and images is not None and input_ids.shape[1] == 1:
